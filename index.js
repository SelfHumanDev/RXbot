...
//Kasi credit gw Mr.A43G & Angga Dwhiqy

//dan juga Subscribe Mr.A43G jgn cuma ganti nama bot
//Subscribe Angga Dwhiwy
...
const BarBarKey = 'Q7M***************9liX' //BELI SENDIRI GOBLOK MODAL DOKIT AJG
const shizukaapi = 'onlyonedeveloper'
const TobzApi = 'BotWeA'
const XteamKey = 'KING******' //BELI SENDIRI GOBLOK MODAL DIKIT AJG
...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6287836332838:+6287836332838\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
vr = '*🍁ANIMESARAN🍁*'
/******** OWNER NUMBER**********/
const ownerNumber = ["6287836332838@s.whatsapp.net"]  //ganti menjadi nomormu
/************************************/
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('(+)','white'), color('Chiwa panda BY Angga','red'), color('(+)','white'), color(' SQAN CODENYA','aqua'), color('SUBSCRIBE YT Angga Dwhiqy','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo kak  @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* yang betah ya di sini kak  dan juga jangan jadi kutu loncat `
				...
				teks = `Selamat tinggal kak @${num.split('@')[0]} Chiwa merindukan mu kak `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
// ANTI LINK GRUP
		        ...
		        if (isGroupAdmins) return reply('Kaka Admin mah bebas ')
		        client.updatePresence(from, Presence.composing)
		        if (mesejAnti.includes(",izinkak")) return reply("Iya kak jangan spam ya ")
		        var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		        reply(`Maaf kak ${sender.split("@")[0]} Grup ini anti link ya kak, siap siap kak di kick chiwa `)
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*Chiwa Harus jadi bot kak *`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
		...
		if (!isGroup && !isCmd) console.log(color(time, "white"), color("[ PRIVATE ]", "aqua"), color(budy, "white"), "from", color(sender.split('@')[0], "red"))
			...
			switch(command) {
				case 'dompet':				if (!isRegistered) return reply(ind.noregis())
		...
		case 'darkneon':
			...
			client.sendMessage(from, darkne, image, {quoted: mek, caption: 'FITUR UPDATE BY Chiwa'})
			...
			client.sendMessage(from, glass, image, {quoted: mek, caption: 'FITUR UPDATE BY Chiwa'})
			await limitAdd(sender)
			break
		case 'attp':
			if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`_Teksnya Mana Boss_\n*Contoh ${prefix}attp Chiwa Cantik*`)
				attp2 = await getBuffer(`https://api.xteam.xyz/attp?file&text=${body.slice(6)}`)
				client.sendMessage(from, attp2, sticker, {quoted: mek})
				break
		case 'narutobanner':
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: ${prefix}narutobanner RiuGanz`)
				nar = body.slice(14)
				narba = await getBuffer(`https://videfikri.com/api/textmaker/narutobanner/?text=${nar}`)
				client.sendMessage(from, narba, image, {quoted: mek, caption: 'FITUR UPDATE BY Chiwa'})
				await limitAdd(sender)
				break
		case 'wait2':
		...
		case 'quranaudio':
				...
				anu = await fetchJson(`https://api.zeks.xyz/api/randomquran`, {method: 'get'})
					...
					client.sendMessage(from, faktaaa, text, {quoted: mek})
					client.sendMessage(from, quu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.nama}.mp3`, quoted: mek})
					await limitAdd(sender) 
					break
		case 'neonime':
			if (!isRegistered) return reply(ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/pinterest?apikey=${apiKey}&query=violet evergarden`)
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/pinterest?apikey=${apiKey}&query=anime girl`)
                    ini_url = ini_url.result
                    buffer = await getBuffer(ini_url)
                    client.sendMessage(from, buffer, image, { quoted: mek })
                    break
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/pinterest?apikey=${apiKey}&query=anime boy`)
                    ini_url = ini_url.result
                    buffer = await getBuffer(ini_url)
                    client.sendMessage(from, buffer, image, { quoted: mek })
                    break
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/pinterest?apikey=${apiKey}&query=senku dr stone`)
                    ini_url = ini_url.result
                    buffer = await getBuffer(ini_url)
                    client.sendMessage(from, buffer, image, { quoted: mek })
                    break
		...
		if (!isRegistered) return reply(ind.noregis())
			...
			case 'elaina':
					...
					client.sendMessage(from, nye, image, { caption: ' kak elaina!!', quoted: mek })
					await limitAdd(sender)
					break
			       case 'jadwaltv':
		if (!isRegistered) return reply(ind.noregis())
...
break
		...
		if (!isRegistered) return reply( ind.noregis())
			...
			if (!isRegistered) return reply(ind.noregis())
                    ...
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa'  })
                    break
					case 'megumin':				//UPDATE
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                 ...
                 if (!isRegistered) return reply( ind.noregis())
                    ...
                    case 'elf':
					if (!isRegistered) return reply(ind.noregis())
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/random/elf?apikey=${apiKey}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    case 'christmas':
                    if (!isRegistered) return reply(ind.noregis())
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/christmas?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    case 'cloud':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/cloud?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    case 'neonligth':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/neonlight?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    case 'holomaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/holographic?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    case 'freefire':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/freefire?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    ...
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/silverplaybutton?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    ...
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/birthdayday?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY Chiwa' })
                    break
                    case '3dmaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/hologram3d?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'airmaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/watercolor?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    ...
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/galaxywallpaper?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'kuemaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/birthdaycake?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'royalmaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/royaltext?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'neon':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/neon?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'sandsummer':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/sandsummer?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'carvedwood':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/carvedwood?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'shadow':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/shadow?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'cup':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/cup?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'undermaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/underwater?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'cup2':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/cup1?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'wolfmetal':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/wolfmetal?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
                    case 'harymaker':
                    if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} MrA43G`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/harrypotter?apikey=${apiKey}&text=${txt}`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: 'FITUR UPDATE BY RIU' })
                    break
					case 'baka':
					if (!isRegistered) return reply(ind.noregis())
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/random/wallnime?apikey=${apiKey}`, {method: 'get'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'FIX BUG BY RIU' })
					break
					case 'smile':				//UPDATE
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'dadu':
			...
			case 'asupan':
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                reply(ind.wait())
                anu = await fetchJson(`http://lolhuman.herokuapp.com/api/asupan?apikey=RiuApikey`)
                asup = await getBuffer(anu.result)
                client.sendMessage(from, asup, video, {mimetype: 'video/mp4', filename: `asupan_bangsa.mp4`, quoted: mek, caption: 'FIX BUG BY RIU'})
                await limitAdd(sender) 
                break
				case 'transfer':
				if (!isRegistered) return reply(ind.noregis())
...
break
                case '1cak':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sekarang semua anggota dapat mengirim perintah')
					} else if (Number(args[0]) === 0) {
						public.splice(from, 1)
						fs.writeFileSync('./src/public.json', JSON.stringify(public))
						reply('Sekarang hanya owner dapat mengirim perintah')
					} else {
		...
		case 'resepmasakan':  
			...
			if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group Elaina :\n\n\`\`\``
					no = 0
	  ...
	  case 'seberapagay':
		...
		case 'infogempa':
               ...
               if (isLimit(sender)) return reply(limitend(pushname2))
                   ...
                   client.sendMessage(from, buffer, image, {quoted: mek, caption: gempa})
                   await limitAdd(sender) 
                   break 
		case 'anime':
                if (!isRegistered) return reply(ind.noregis())
	...
	case 'hobby':
		...
		break
				...
				logo = fs.readFileSync(`./src/logo.jpg`)
		            await client.sendMessage(from, logo, image, {quoted: mek, caption: ind.menu(pushname, prefix, getLevelingLevel, getLevelingXp, sender, reqXp, _registered, uangku)})
					break
				case 'donasi':
				case 'donate':
				if (!isRegistered) return reply(ind.noregis())
		...
		if (budy.includes(`bot`)) {
                  reply(`Iya Elaina disini ketik ,help ya kak`)
                  }
		...
		if (budy.includes(`Bot`)) {
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam *${prefix} catatan Elaina!*`)
		...
		//client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[Elaina]','aqua'), ('Command Tidak Ditemukan!!','red'), color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
